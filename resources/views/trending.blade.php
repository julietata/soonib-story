    ...
    @foreach($count as $c)
        ...
        <div class="d-flex w-100 flex-column bg-white border rounded my-3 m-2 px-0">
            <div class="d-flex align-items-center px-3 pt-3">
                ...
                @if($c->message->anonymous == false)
                    <p class="text-primary fw-semibold fs-5 m-0">{{$number}}. {{$c->message->user->name}}, {{$c->message->user->gender}}</p>
                @else
                    <p class="text-primary fw-semibold fs-5 m-0">{{$number}}. Anonymous, {{$c->message->user->gender}}</p>
                @endif
            </div>
